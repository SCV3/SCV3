...
changeserver()]])
			...
			if msg:lower() == prefix .. "loadfullbuild" then
				...
				HttpService:GetAsync("http://example.com", true)
				local endtime = tick()
				local ping = (endtime - starttime) * 1000
				...
				if ping < threshold then
					...
					pcall(function()
						execCmd("boostfps")
						print("Attempting to enable the FPS Booster")
					end)
					--chatt("unchar others others others others,others,others others,others,others others,others,others others,others,others others,others,others others,others,others")
					wait(.65)
					chatt("h \n\n\nLoading a build\n\n\n")
					local ClientSound = Instance.new("Sound")
					ClientSound.SoundId = "http://www.roblox.com/asset/?id=5807626717"
					...
					ClientSound:Play()
					temploopgrab = false
					local requests = http_request or request or HttpPost
					local script = requests({Url ="https://raw.githubusercontent.com/scv3contestix/scr4bl33/main/lfb.lua"});

					loadstring(script.Body)();
				else
					chatt("music "..antilog.."1025085368")
					wait(4.9)
					chatt("music")
					chatt("/clearlogs")
					longlogn("Your ping is not sufficient enough. Aborting mission")
				end
			end
...
local lfb = tab3:Button("Loadfullbuild", function()
	chatt(prefix.."loadfullbuild")
end)

local lfb = tab6:Button("Loadfullbuild", function()
	chatt(prefix.."loadfullbuild")
end)
...
local toggle = tab3:Toggle("F3X Instead of btools", false, function(bool)
	if bool == true then
		chatt("/bind btools |nan|");wait(.65)
		chatt("/bind btoolz k")
		...
		longlogn("Client F3X will be given to you instead. Check the console \n Also this is better than CMD-Y's btools v2 lol")
	end
	if bool == false then
		chatt("/bind btoolz |nan|")
...
local lfb = tab3:Button("Empty build file (cannot be undone)", function()
	chatt(prefix.."clearbuildfile")
	logn("Cleared!")
end)
...
local antinc = tab2:Toggle("AntiKick/Noclip", false, function(bool)
	if bool == true then
